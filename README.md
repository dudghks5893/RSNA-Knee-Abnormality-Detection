...
### Public LB 직접 예측 결과
- **Public LB: 0.904**
- Top-24 최종 이미지 모델 없이, 전체 MRI 특징 통합 모델 자체의 12개 질환 예측을 그대로 제출
- 기존 Exp11B 고정 Wide9 모델 0.872 대비 **+0.032**
- 현재 최고 Top-24 warm-start 모델 0.907 대비 **-0.003**
...
## Experiment 50 — 전체 MRI 모든 슬라이스 특징 통합 모델의 직접 Public LB 검증

### 목적
환자별 Top-24를 선택해 다른 최종 이미지 모델로 넘기지 않고,
전체 MRI의 모든 Series와 모든 slice 특징을 계층적으로 통합하는 모델 자체가
hidden test에서 어느 정도 일반화되는지 확인했다.

### Hidden-test 추론
```text
hidden test 전체 MRI
→ 모든 Series / 모든 slice를 3-slice window로 구성
→ Exp11B에서 무릎 MRI에 맞게 학습된 DINOv2-Base로 각 window 특징 추출
→ train feature cache와 동일하게 float16 저장 정밀도 재현
→ 전체 MRI 특징을 질환별 window/series attention으로 통합
→ 12개 질환 직접 예측
```

Top-24 선택, NMS, B3A/B3B 최종 모델은 사용하지 않았다.

### 실행 확인
- visible test 3 studies 정상 처리
- window 수: **284 / 135 / 138**
- decode error: **0**
- submission contract: **PASS**

### 결과
- Fold2 Macro ROC-AUC: **0.954167**
- Fold2 Weak-6 AUC: **0.915278**
- **Public LB: 0.904**

### 비교
- 기존 Exp11B 고정 Wide9: 0.872
- 동일 Top-24 clean-start 최종 모델: 0.903
- **전체 MRI 직접 예측: 0.904**
- **Top-24 warm-start 최종 모델: 0.907**

### 인사이트
- 전체 MRI 특징 통합 모델 자체가 0.904까지 올라가면서,
  성능 향상이 단순히 “24장을 골라서 다시 학습했기 때문”만은 아니라는 점이 확인됐다.
- 전체 MRI에서 질환별 중요도를 학습하고 여러 Series를 통합하는 단계 자체가
  hidden test에서 매우 강한 예측력을 가지고 있다.
- B3A는 여기서 추가로 +0.003을 얻었으므로,
...
| Exp16B-2 | 전체 MRI 모든 슬라이스 특징을 질환별로 통합해 직접 예측 | **0.954167** | 0.915278 | **0.904** |
